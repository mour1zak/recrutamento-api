...
// não funcionou em nenhuma sintaxe testada (achado da revisão técnica, ressalva
...
// Achado crítico da auditoria técnica (C1): sem isso, qualquer pessoa
...
// e permanentes no histórico do Git. Corrige achado da revisão técnica (N4):
...
export const envValidationSchema = Joi.object({
  ...
  .custom((value, helpers) => {
    ...
    if (value.JWT_SECRET === value.API_KEY) {
      ...
      // achado da revisão técnica (N7) — a mensagem passada via `context` num
